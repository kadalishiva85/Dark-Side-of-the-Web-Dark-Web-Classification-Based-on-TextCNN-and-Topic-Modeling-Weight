...
from Remote_User.models import ClientRegister_Model,predict_web_classification,detection_ratio,detection_accuracy
...
def View_Predicted_Web_Classification_Type_Ratio(request):
    detection_ratio.objects.all().delete()
    ratio = ""
    kword = 'Threat Found'
    print(kword)
    obj = predict_web_classification.objects.all().filter(Q(Prediction=kword))
    obj1 = predict_web_classification.objects.all()
    count = obj.count();
    count1 = obj1.count();
    ...
    ratio12 = ""
    kword12 = 'Threat Not Found'
    print(kword12)
    obj12 = predict_web_classification.objects.all().filter(Q(Prediction=kword12))
    obj112 = predict_web_classification.objects.all()
    count12 = obj12.count();
    ...
    return render(request, 'SProvider/View_Predicted_Web_Classification_Type_Ratio.html', {'objs': obj})
...
def View_Prediction_Web_Classification_Details(request):
    obj =predict_web_classification.objects.all()
    return render(request, 'SProvider/View_Prediction_Web_Classification_Details.html', {'list_objects': obj})
...
def Download_Trained_DataSets(request):
    ...
    for my_row in data:
        ...
        ws.write(row_num, 12, my_row.Url, font_style)
